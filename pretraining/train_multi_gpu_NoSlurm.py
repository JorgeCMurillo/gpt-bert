...
def parse_arguments():
    ...
    parser.add_argument("--wandb_project", default="GPT-BERT Trial", type=str, help="Name of the WandB project to log into.")
    parser.add_argument("--wandb_entity", default="jcmurillo", type=str, help="The entity to log to on WandB (typically your wandb username).")
    ...
    parser.add_argument("--checkpoint_filename", default=None, type=str, help="The checkpoint directory to resume training from.")
    ...
    args = parser.parse_args()
        # --- ADD THIS BLOCK ---
    print("\n--- 1. After parsing arguments ---", flush=True)
    print(f"args.max_steps = {args.max_steps} (Type: {type(args.max_steps)})", flush=True)
    # --- END BLOCK ---
...
def setup_training(args, tokenizer):
    ...
    # Step 1: Set up the core distributed variables from the environment
    args.world_size = int(os.environ["WORLD_SIZE"])
    args.rank = int(os.environ["RANK"])
    args.local_rank = int(os.environ["LOCAL_RANK"])
    ...
    # Step 5: The rest of your setup logic
    assert args.world_size % args.hybrid_denominator == 0
    ...
    if is_main_process():
        print(f"Rank {args.rank} is using dataset type: {args.dataset_type}")
...
def load_config(args):
        # --- ADD THIS BLOCK ---
    print("\n--- 2. Entering load_config ---", flush=True)
    print(f"args.max_steps BEFORE loading = {args.max_steps} (Type: {type(args.max_steps)})", flush=True)
    # --- END BLOCK ---
    ...
    for k, v in config.items():
        setattr(args, k, v)
    # --- ADD THIS BLOCK ---
    print("\n--- 3. Exiting load_config ---", flush=True)
    print(f"args.max_steps AFTER loading = {args.max_steps} (Type: {type(args.max_steps)})", flush=True)
    # --- END BLOCK ---
    return args
...
def get_batch(dataloader, device, global_step):
    ...
    # Check if the dataset object has the set_global_step method before calling it
...
def training_epoch(model, ema_model, train_dataloader, valid_dataloader, optimizer, scheduler, global_step, epoch, args):
    ...
    # get the first batch
    input_ids_, attention_mask_, target_ids_, mask_p_ = get_batch(train_iterator, args.device, global_step)
    ...
    for local_step in tqdm(range(num_steps), desc="Train iteration", initial=global_step, total=args.max_steps, disable=not is_main_process()):
        ...
        if not torch.isfinite(loss).all():
            print(f"!!! RANK {get_rank()}: Unstable loss detected: {loss.item()} at step {global_step}", flush=True)
            # The line below is crucial. It stops this one GPU from hanging the others.
            # Find a way to exit cleanly or raise an exception that stops all processes.
        ...
        if is_main_process():
            # This will print every single step. We are looking for what happens when it hits the limit.
            print(f"Checking exit condition: global_step = {global_step}, max_steps = {args.max_steps}", flush=True)
        # --- END DIAGNOSTIC BLOCK ---

        # Exiting the training due to hitting max steps
        if global_step >= args.max_steps:
            if is_main_process():
                print(f"\n--- STOP CONDITION MET. Exiting training. ---\n", flush=True)
            return global_step
    return global_step
