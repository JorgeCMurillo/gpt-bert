...
class MaskedDataset(torch.utils.data.Dataset):
    ...
    # In dataset.py, replace the __getitem__ method in BOTH dataset classes

    # def __getitem__(self, index):
    #     # --- DEBUGGING CODE ---
    #     # This bypasses all file I/O and processing to test the
    #     # stability of the training loop itself.
    #     input_ids = torch.randint(0, self.args.vocab_size, (self.seq_length,))
    #     target_ids = input_ids.clone()
    #     attention_mask = torch.ones(self.seq_length, self.seq_length, dtype=torch.bool)

    #     # This logic handles the different return formats for each dataset class
    #     if "Masked" in self.__class__.__name__:
    #         return input_ids, target_ids, ~attention_mask, torch.tensor(0.15)
    #     else: # This will be for CausalDataset
    #         return input_ids, target_ids, ~attention_mask.tril(), torch.zeros([])
    def __getitem__(self, index):
                # --- BREADCRUMB 1 ---
        # Let's see if we even start __getitem__ on the problematic rank
        rank_id = self.args.rank if hasattr(self.args, 'rank') else -1
        print(f"[Rank {rank_id}] GETITEM Start: index {index}", flush=True)
        ...
        # --- BREADCRUMB 2 ---
        print(f"[Rank {rank_id}] GETITEM: Calling masking_strategy", flush=True)
        
        mask_ratios, replacement_tokens = self.masking_strategy(tokens, self.mask_counts[index][:seq_length])
        print(f"[Rank {rank_id}] GETITEM: Calling apply_mask", flush=True)
        ...
        while self.seq_length - input_ids.size(0) > 1:
            ...
            if current_iteration >= max_iterations:
                print(f"!!! RANK {rank_id}: Breaking while loop in __getitem__ after {max_iterations} iterations to prevent hang.", flush=True)
                break
            current_iteration += 1
        ...
        print(f"[Rank {rank_id}] GETITEM End: index {index}", flush=True)
    ...
    def apply_mask(self, input_ids, mask_ratios, replacement_ids):
        ...
        if not torch.isfinite(mask_ratios).all():
            rank_id = self.args.rank if hasattr(self.args, 'rank') else -1
            print(f"!!! RANK {rank_id}: NaN DETECTED IN mask_ratios BEFORE topk !!!", flush=True)
            print("Problematic mask_ratios tensor:", mask_ratios, flush=True)
            exit() # Exit cleanly to see the message
